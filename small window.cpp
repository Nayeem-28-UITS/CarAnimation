...
void drawSky() {
    ...
    bar(0, 0, getmaxx(), 320);           // Sky fills the upper half of the screen
}
...
void moveCar(int speed) {
    ...
    int cloudOffset = 0;
        ...
        char speedText[50]; // Buffer to store the speed text
    ...
    while (true) {
        ...
        for (int i = 0; i < 6; i=i+2) {
            drawBuilding(60 + i * 220, roadY - 17); // Draw buildings
        }
        for (int i = 0; i < 6; i+=2) {
            drawTree(350 + i * 220, roadY - 106); // Draw tree
        }


          // Draw tree on the right side of the building
        drawCar(xLeft, carY);

                // Draw the moving car

        // Display the current speed
        settextstyle(BOLD_FONT, HORIZ_DIR, 3); // Set font to bold, horizontal direction, size 3
        setcolor(WHITE);
        sprintf(speedText, "Speed: %d", speed);
        outtextxy(10, 10, speedText);

        // Draw "V"-shaped grass between flowers
        for (int i = 1; i < 16; i=i+2) {
            drawVGrass(60 + i * (getmaxx() / 16), grassY +120);

        }

            // Function to draw a row of flowers

        int flowerSpacing = getmaxx() / 16; // Space between flowers
        for (int i = 0; i < 16; i=i+2) {      // Draw a maximum of 14 flowers
            drawFlower(60+i * flowerSpacing, grassY+120);
            }
...
int main() {
    int gd = DETECT, gm;
    initwindow(1366, 720, "Car Animation");
